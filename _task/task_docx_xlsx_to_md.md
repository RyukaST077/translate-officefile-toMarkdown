...
- 実行タスク:
  - [x] 出力フォルダの連番付与ルールを実装する（`_md`, `_md_2`, `_md_3`…）
  - [x] 出力ファイル名の連番付与ルールを実装する（`a.md`, `a_2.md`, `a_3.md`…）
  - [x] 相対パス維持の出力パス生成を実装する
  - [x] 再帰探索で `.docx`, `.xls`, `.xlsx` を抽出する（大文字小文字無視）
  - [x] 除外ルールを実装する（`~$*` と入力配下に存在する `*_md*` フォルダ）
  - [x] 例外系（アクセス不可パス）の扱いを明確化し、呼び出し側へ伝播できる形にする
- 受け入れ基準:
  - [x] 入力フォルダと同階層に、上書きしない出力フォルダが決定できる
  - [x] 同名衝突時の `.md` 連番が正しく決まる
  - [x] 対象拡張子のみが収集され、除外ルールが機能する
...
- 実行タスク:
  - [x] `MarkItDown(enable_plugins=False)` を用いた変換処理を実装する
  - [x] `.xlsx` に対して `openpyxl` で画像有無を検出する（検出不能時はスキップ＋ログ用情報を返す）
  - [x] 画像のMarkdown埋め込み（`![](...)` など）を除去する後処理を実装する
  - [x] 画像検出時の注記ルールを **要件優先** で実装する（例：シート見出しに「画像あり」を付与）
  - [x] 数式は表示値のみ（MarkItDown結果を採用）とし、取得不可時に警告情報を返す
  - [x] Excel複数シートが `## SheetName` で分割されているか検証し、必要なら後処理で補正する
- 受け入れ基準:
  - [x] `.docx/.xls/.xlsx` が1ファイル→1 `.md` に変換される
  - [x] 画像は埋め込まれず、検出できた場合のみ注記が付く
  - [x] 数式は結果値のみが出力され、取得不可時は警告情報が得られる
  - [x] Excelの複数シートが1つのmd内に見出し区切りで出力される
...
- 実行タスク:
  - [x] 変換の開始/終了を制御する `ConversionController` を実装する
  - [x] ワーカースレッドで変換を実行し、進捗イベント（n/total, 処理中ファイル）をUIに通知する
  - [x] 入力フォルダ選択時に出力先プレビューを自動更新する
  - [x] 実行中は入力再選択と実行ボタンを無効化する
  - [x] `conversion.log` を出力フォルダ配下に生成し、成功/失敗/警告を記録する
  - [x] 完了時に成功/失敗件数・出力先・ログ案内を表示するダイアログを実装する
- 受け入れ基準:
  - [x] UIがフリーズせず、進捗バーとステータスが更新される
  - [x] 変換失敗があっても処理が継続し、ログに記録される
  - [x] 完了ダイアログに成功/失敗件数と出力先が表示される
